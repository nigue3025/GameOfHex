...
#include<map>
#include<time.h>
////#define RANDOM_VS_MONTECARLO
class Node
{

public:
	enum EnumNodePlayerStatus :short { Empty, FirstPlayer, SecondPlayer };
	EnumNodePlayerStatus NodeStatus;
	...
	Node(int i, int j, int size, std::vector<int>& edgelist)
	{
		...
		MyEdgeList = edgelist;
		No = i*size + j;
		NodeStatus = EnumNodePlayerStatus::Empty;
	};
...
public:
	...
	Node::EnumNodePlayerStatus LastMovedPlayer()
	{
		if ((placeCount + 1) % 2 == 1)
			return Node::EnumNodePlayerStatus::SecondPlayer;
		else
			return Node::EnumNodePlayerStatus::FirstPlayer;

	}

	//Dislay the player of the game with the next move
	Node::EnumNodePlayerStatus NextMovePlayer()
	{
		if ((placeCount + 1) % 2 == 1)
			return Node::EnumNodePlayerStatus::FirstPlayer;
		else
	...
	bool place(std::string cordStr, Node::EnumNodePlayerStatus NodeStatus, std::string& msg)
	{
		msg = "";
		...
		else if (NodeMap[cordStr]->NodeStatus != Node::EnumNodePlayerStatus::Empty)
		{
	...
	bool JudgeWinner(Node& JustPlacedNode, std::string& msgOut, HexGraph::EnumPlayResult & result, Node::EnumNodePlayerStatus player)
	{
		player = JustPlacedNode.NodeStatus;
		if (JudgeWinner(JustPlacedNode, msgOut, player))
		{
			if (player == Node::EnumNodePlayerStatus::FirstPlayer)
	...
	bool JudgeWinner(Node& JustPlacedNode, std::string& msgOut, Node::EnumNodePlayerStatus& player)
	{
...
private:
	...
	std::string showPlayer(Node::EnumNodePlayerStatus status)
	{
		if (status == Node::EnumNodePlayerStatus::FirstPlayer)
			return "Player 1(Blue) ";
		else if (status == Node::EnumNodePlayerStatus::SecondPlayer)
			return "Player2(Red) ";
		else
			return "error";
	}
...
{
public:
	Node::EnumNodePlayerStatus Player;
	AI(Node::EnumNodePlayerStatus player)
	{
		Player = player;
		srand(time(NULL));
	}
...
{
public:
	RandomMethod(Node::EnumNodePlayerStatus player) :AI(player)
	{
	};
...
public:
	...
	MonteCarloMethod(int simulateNo, Node::EnumNodePlayerStatus player) : AI(player)
	{
...
private:
	...
	HexGraph::EnumPlayResult runSingleSimulationGame(HexGraph& stmHxGrph, Node::EnumNodePlayerStatus thePlayer)
	{
		int moveCount = 0;
		HexGraph::EnumPlayResult rslt;
	...
	//	int getBestMove_Multithread(HexGraph& givenHexGraph)
	//	{
	//		int bestWinCount = -1;
	//		int WinCount_secondPlan = -1;
	//		Node* bestMove = nullptr;
	...
	//			return givenHexGraph.RemainedEmptyNode.begin()->second->No;
	//
	//		int thdSz = givenHexGraph.RemainedEmptyNode.size();
	//		
	//		int* winnerWinCountBoard = new int[thdSz];
	//		int* loserWinCountBoard= new int[thdSz];
	//		std::thread* thds = new std::thread[thdSz];
	//		std::vector<std::string> CordeinateStrLst;
	//		
	//		//std::vector<std::thread> thds;
	//		
	//
	//		for (auto &node : givenHexGraph.RemainedEmptyNode)
	//		{
	//			if (node.second == NULL)
	//				givenHexGraph = givenHexGraph;
	//
	//			Node* nodek = node.second;
	//			nodek = nodek;
	//			auto cordinaeStr = node.second->showCordinate();
	//			CordeinateStrLst.push_back(cordinaeStr);
	//		}
	//
	//		CordeinateStrLst = CordeinateStrLst;
	//
	//
	//		for (int i = 0;i < thdSz;i++)
	//		{
	//			auto NewGivenHexGraph = givenHexGraph.clone();
	//			SimulationThread( NewGivenHexGraph, i, CordeinateStrLst[i], winnerWinCountBoard, loserWinCountBoard);
	//
	//		}
	//
	//		/*for (int i = 0;i < thdSz;i++)
	//		{
	//			auto NewGivenHexGraph = givenHexGraph.clone();
	//			thds[i] = std::thread(&MonteCarloMethod::SimulationThread, this, NewGivenHexGraph, i,CordeinateStrLst[i] , winnerWinCountBoard, loserWinCountBoard);
	//		}
	//		for (int i = 0;i < thdSz;i++)
	//			thds[i].join();
	//*/
	...
	//		//Get best move
	//
	//		givenHexGraph.RemainedEmptyNode = givenHexGraph.RemainedEmptyNode;
	//		givenHexGraph.RemainedEmptyNode = givenHexGraph.RemainedEmptyNode;
	//
	//		if (givenHexGraph.RemainedEmptyNode.size() == 1) //Remained last move only. No longer to be simulated with other moves
	//			givenHexGraph.RemainedEmptyNode = givenHexGraph.RemainedEmptyNode;
	...
	//		for (int i = 0;i < thdSz;i++)
	//		{
	//			if (winnerWinCountBoard[i] > bestWinCount)
	//			{
	//				bestWinCount = winnerWinCountBoard[i];
	//				bestMove = givenHexGraph.RemainedEmptyNode[i];
	//			}
	//			if (loserWinCountBoard[i] > WinCount_secondPlan)
	//			{
	//				WinCount_secondPlan = loserWinCountBoard[i];
	//				BestMove_secondPlan = givenHexGraph.RemainedEmptyNode[i];
	//			}
	//		}
	//
	//		//for (auto &candidate : givenHexGraph.RemainedEmptyNode) //Run simulation for evaluating each empty move to get the best move
...
std::set<Node::EnumNodePlayerStatus> RemainedPlayer = { Node::EnumNodePlayerStatus::FirstPlayer,Node::EnumNodePlayerStatus::SecondPlayer };

int main()
{
	int boardSize = 1;
	int simulateNo = 1000;
	std::string inputBoardSizeStr;
	std::string UserSelectedPlayerStr;
	Node::EnumNodePlayerStatus UserSelectedPlayer;

	std::cout << "Enter board Size(a integer number):" << std::endl;
	std::cin >> boardSize;


	AI *SelectedAiPlayer, *RandomAI;


	while (true)
	{
		std::cout << "Play as 1st Blue player(1) or 2nd Red player(2)?:" << std::endl;
		std::cin >> UserSelectedPlayerStr;
		if (UserSelectedPlayerStr == "1")
		{
			UserSelectedPlayer = Node::EnumNodePlayerStatus::FirstPlayer;
			SelectedAiPlayer = new MonteCarloMethod(simulateNo, Node::EnumNodePlayerStatus::SecondPlayer);
			std::cout << "You are player1(Blue)\n";
			break;
		}
		else if (UserSelectedPlayerStr == "2")
		{
			UserSelectedPlayer = Node::EnumNodePlayerStatus::SecondPlayer;
			SelectedAiPlayer = new MonteCarloMethod(simulateNo, Node::EnumNodePlayerStatus::FirstPlayer);
			std::cout << "You are player2(Red)\n";
			break;
		}
		else
		{
			std::cout << "input error, please try again(Reminder: key-in 1 or 2 to select your play sequence)\r\n";
		}
	}

	HexGraph hexGraph = HexGraph(boardSize);

#ifdef RANDOM_VS_MONTECARLO
	RandomAI = new RandomMethod(UserSelectedPlayer);
#endif

	std::cout << "Board sample displayed with node cordinates\n" << hexGraph.showArrangedNodesCordinates() << std::endl;
	std::cout << "Board sample displayed with node id\n" << hexGraph.showArrangedNodesId() << std::endl;
	std::cout << "Board sample displayed with current status\n" << hexGraph.showArrangedPlayerNodes() << std::endl;

	while (true)
	{
		std::string inputStr;

		if (hexGraph.NextMovePlayer() == UserSelectedPlayer)
		{
			std::cout << "Your turn now! As player " << hexGraph.NextMovePlayer() << "\n";
			std::cout << "Place your node with cordinates. (Input the cordinate with the format (i,j), parethesis included)\n";

#ifdef RANDOM_VS_MONTECARLO
			inputStr = RandomAI->generateNextMove(hexGraph);
#endif
#ifndef RANDOM_VS_MONTECARLO
			std::cin >> inputStr;  //Cordinate format with (i,j) is strictly limited. (Parethesis included)
#endif
		}
		else
		{

			std::cout << "AI turn. Please stand by...\n";
			clock_t t1, t2;
			t1 = clock();
			inputStr = SelectedAiPlayer->generateNextMove(hexGraph);
			t2 = clock();
			std::cout << "AI generated move \r\n" << inputStr << std::endl;
			std::cout << ((double)(t2 - t1)) / 1000.0 << "second(s) elapsed" << std::endl;
		}
		std::string msg;
		if (!hexGraph.place(inputStr, hexGraph.NextMovePlayer(), msg))
			std::cout << "place error: " + msg << std::endl;
		else
		{
			...
			{
				std::cout << currPlayer << "win" << std::endl;
				break;
			}
		}
	}
